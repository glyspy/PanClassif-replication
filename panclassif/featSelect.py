def featSelect(homepath,cancerpath,normalpath,k=5):
    ...
    cancerfiles = [f for f in listdir(cancerpath) if isfile(join(cancerpath, f)) and not f.startswith('.')]
    normalfiles = [f for f in listdir(normalpath) if isfile(join(normalpath, f)) and not f.startswith('.')]
    names = []
    ...
    print("Cancer files:", cancerfiles)
    ...
    for index in range(len(cancerfiles)):
        ...
        Cancer = pd.read_csv(cancerpath+'/'+cancerfiles[index], header=None, index_col=None, encoding='utf-8')
        Normal = pd.read_csv(normalpath+'/'+normalfiles[index], header=None, index_col=None, encoding='utf-8') 
