def step1(homepath,names,cancer):
	import numpy as np
	import pandas as pd
	...
	"""# Frequency Check"""
	data = pd.read_csv(homepath+"/cancer/"+cancer+".csv.gz",header=None)
	one_col = data.iloc[:,0:1] # takes the first column (genes)
	one_col = one_col.drop(one_col.index[0]) # removes first row (because empty)
